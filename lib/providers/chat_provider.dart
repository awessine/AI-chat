...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> updateSettings({
    ...
    try {
      _log('Updating settings: provider=$provider, modelId=$modelId');

      // Обновляем конфиг клиента (_api) — без замены инстанса
      // Маппим провайдера в baseUrl
      final newBaseUrl = provider == 'vsegpt'
          ? 'https://api.vsegpt.ru/v1'
          : 'https://openrouter.ai/api/v1';

      // Метод ниже — это расширение твоего OpenRouterClient (см. следующий блок).
      // Он должен изменить baseUrl и/или ключ, не ломая текущий клиент.
      await _api.updateConfig(
        baseUrl: newBaseUrl,
        apiKey: apiKey, // если null — клиент сам оставит старый ключ
      );

      // Перечитываем модели и баланс под новым провайдером
      await _loadModels();
      await _loadBalance();

      // Выбираем модель:
      // - если передали modelId и она есть в списке — используем её
      // - иначе, если текущая _currentModel отсутствует у нового провайдера — ставим первую доступную
      if (modelId != null && modelId.isNotEmpty) {
        final exists = _availableModels.any((m) => m['id'] == modelId);
        _currentModel = exists
            ? modelId
            : (_availableModels.isNotEmpty
                ? _availableModels.first['id'] as String
                : null);
      } else {
        final stillExists = _currentModel != null &&
            _availableModels.any((m) => m['id'] == _currentModel);
        if (!stillExists) {
          _currentModel = _availableModels.isNotEmpty
              ? _availableModels.first['id'] as String
              : null;
        }
      }
      ...
      notifyListeners();
      _log(
          'Settings updated. baseUrl=${_api.baseUrl}, currentModel=$_currentModel');
    } catch (e, st) {
      _log('Error updating settings: $e');
      _log('Stack trace: $st');
      rethrow;
    }
  }
